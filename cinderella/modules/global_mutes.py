...
def gmute(bot: Bot, update: Update, args: List[str]):
    ...
    message.reply_text(
        f"{mention_html(user_chat.id, user_chat.first_name)} won't be talking again anytime soon.",
        parse_mode=ParseMode.HTML,
    )
...
def ungmute(bot: Bot, update: Update, args: List[str]):
    ...
    message.reply_text(
        f"{mention_html(user_chat.id, user_chat.first_name)} has been un-gmuted.",
        parse_mode=ParseMode.HTML,
    )
