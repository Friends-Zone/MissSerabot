...
def check_flood(bot: Bot, update: Update) -> str:
    ...
    if soft_flood := sql.get_flood_strength(chat.id):
        ...
        reply = f"Wonderful, I don't like your flooding. Get out! {mention_html(user.id, user.first_name)} has been kicked!"


    else:
        chat.kick_member(user.id)
        reply = f"Frankly, I like to leave the flooding to natural disasters. {mention_html(user.id, user.first_name)} has been banned!"
    ...
    if args:
        ...
        if val in ["off", "no", "0"]:
            ...
            if amount <= 0:
                ...
                message.reply_text(f"Anti-flood has been updated and set to {amount}")
                return "<b>{}:</b>" \
                       "\n#SETFLOOD" \
                       "\n<b>• Admin:</b> {}" \
...
def __chat_settings__(chat_id, user_id):
    ...
    if soft_flood:
        return f"Anti-flood is set to `{limit}` messages and *KICK* if exceeded."
    else:
        return f"Anti-flood is set to `{limit}` messages and *BAN* if exceeded."
__help__ = """
