...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        ...
        bot.sendMessage(
            chat.id,
            f"🔨 {mention_html(member.user.id, member.user.first_name)} was banned by {mention_html(user.id, user.first_name)}!",
...
def kick(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if res := chat.unban_member(user_id):
        ...
        bot.sendMessage(
            chat.id,
            f"{mention_html(member.user.id, member.user.first_name)} has been Kicked by {mention_html(user.id, user.first_name)}!",
            parse_mode=ParseMode.HTML,
        )
